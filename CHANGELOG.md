...
### fix
- **生命周期持久化与防数据丢失**：移除了 `reader-persistence.js` 中不可靠且可能引起并发写坏数据的 `beforeunload` 监听器，进度与阅读时长存储现在完全依靠 `visibilitychange` 与安全防抖，确保标签页被强制关闭时数据绝不丢失。
- **CFI 损坏导致白屏死锁修复**：在 `reader-runtime.js` 的 `_initializeBook` 中为 `rendition.display(displayCfi)` 加入异常捕获。当存储的历史 CFI 无法解析（如同名电子书排版更新、节点丢失等场景）时，主动拦截错误并降级回退至 `display()` 打开首页，彻底杜绝部分受损书籍因报错中断而永远无法打开的严重漏洞。
- **书籍导入过程的 IndexedDB 竞态流产拦截**：修正了 `home.js` 中上传本地 EPUB 文件的逻辑，现在必须 `await EpubStorage.storeFile` 完成 IDB 落盘后再跳转阅读器，并配套增加了「导入中」的同步视觉遮罩，彻底杜绝了因网络加载截断、标签页导航过快导致的 “有书籍记录却无本地文件” 的缓存碎片化漏洞。
...
### fix
- **交叉重排锚点隔离**：resize 防抖任务被字号、行高等更新 reflow 作废时，按 timer 身份清理其旧 CFI 与 rendition 状态；下一次 resize 不再复用上一个视口锚点，迟到 timer 也不会误清更新任务。

### test
- 增加 resize 排队后被字体重排作废、再 resize 时必须使用最新稳定 CFI 的回归；单元覆盖增至 293 项。
...
### refactor
- **速度元数据瘦身**：移除 v2.2 遗留且早已没有读取者的 `sessions` / `sessionCount` 字段，统一收敛为加权进度采样与正文计数；读取旧 bookMeta 时会保留有效速度数据并自然丢弃废弃字段。
- 同步精简 Runtime 默认状态、存储 patch/归一化路径、测试夹具与当前架构接口说明，历史 CHANGELOG 记录保持不变。

### test
- 增加旧速度会话明细归一化丢弃回归；单元覆盖增至 292 项。
...
### fix
- **IndexedDB 同步连接异常可恢复**：`indexedDB.open()` 因安全策略或环境限制同步抛错时同样进入连接失败冷却，但不再把永久 rejected Promise 留在连接缓存；环境恢复后下一次调用可以真正重连。

### test
- 增加 `indexedDB.open()` 同步抛错、缓存清理、失败计数与后续重连回归；单元覆盖增至 291 项。
...
### fix
- **阅读时长原子累加**：Reader 改为只提交本标签页尚未落盘的秒数，由 `EpubStorage.addReadingTime()` 在 bookMeta 资源锁内累加；多个标签页同时阅读同一本书时不再用各自旧总时长互相覆盖。
- **时长生命周期收口**：定时、页面隐藏、关闭与切书统一调用 `flushReadingTime()`；写入前转移增量批次，失败会退回待提交秒数，成功则用权威总时长校准当前展示。
...
### fix
- **速度样本原子累加**：Reader 不再用页面内旧 `cachedSpeed` 计算总数后覆盖；每次会话只提交加权秒数与进度增量，由 `EpubStorage.addReadingSpeedSample()` 在 bookMeta 资源锁内累加并返回权威速度，多个标签页同时阅读同一本书时不丢样本或正文计数字段。
- **正文扫描清理隔离**：章节正文已成功统计后，单独的 `unload()` 清理异常只记录告警，不再把完整计数误标为失败。

### test
- 增加并发速度增量合并与正文计数字段保留回归；单元覆盖增至 287 项。
...
### fix
- **布局往返精确保位**：`setLayout()` 优先使用与主 CFI 绑定的页内 `restoreCfi`，并在字体/行距稳定后只重放同一个 CFI 一次；分页与滚动布局往返不再落到重排前分页对应的错误列。迟到布局任务在重放前校验操作代次，不操作已销毁 rendition。
- **错误页可恢复导入**：加载错误与跨标签页删除不再移除 epub.js 的固定 `#epub-viewer` 挂载节点；错误提示会独立隐藏 viewer，下一次导入清除 wrapper 后可在原 Reader 页面重新挂载。

### test
- 真实 Chrome E2E 扩展为百分比导航、布局往返、resize、目录、全文搜索、删除后原页重导入，以及 Home 确认框清空当前书和孤立资源的完整流程；新增错误页挂载节点与布局页内锚点回归。单元覆盖增至 286 项。
...
### feat
- **本书历史平均字速**：Reader 在 locations 就绪后于后台逐章统计混合语言正文量，将既有有效进度样本换算为字/分钟；中日韩字符逐字计数，连续英文/数字按词计数，旧书按需惰性补齐且不阻塞首屏。
- **Reader 与书架统一展示**：阅读器底栏增加历史平均字速，书架卡片复用单次 `bookMeta` 读取展示“字/分”；正文计数或样本不足时统一降级为“估算中”。

### fix
- `saveReadingSpeed()` 改为字段级 patch，正文计数与速度采样并发写入时保留彼此字段；切书取消旧扫描，章节失败不落盘不完整计数。

### test
- 增加混合语言计数、字速公式、存储 patch、后台章节扫描、缓存命中、Reader 文案和书架安全渲染回归；单元覆盖增至 285 项。
...
### fix
- **书签/高亮原子更新**：功能模块不再执行“读取整表后覆盖整表”；单个 CFI 的新增、修改、删除现在在存储层资源锁内完成读改写，多个 Reader 同时操作同一本书时保留各自记录。
- **迟到操作取消**：书签或高亮等待存储锁期间若已切书，其 mutator 返回取消信号且不写 storage，避免旧上下文污染已离开的书籍。

### test
- 增加并发高亮/书签 CFI 合并回归；真实浏览器流程增加书签保存及关闭重开恢复验证。单元覆盖增至 279 项。
...
### fix
- **跨页面同资源写入串行**：在书级共享锁内增加 bookMeta、高亮、书签、封面、locations 与文件各自的独占锁；两个 Reader 同时更新同一本书时不再因共享锁并行执行读改写而丢失字段。
- **缓存读取一致性**：`getFile()` 的删除标记检查、IndexedDB 读取和 LRU timestamp touch 进入同一个锁事务；LRU 淘汰也持有书级独占锁，避免缓存打开与淘汰交错。级联删除失败遗留但已有删除标记的文件不会被 URL 直开路径重新载入。

### test
- 增加资源锁模式与 tombstoned EPUB 读取屏蔽回归；单元覆盖增至 278 项。
...
### fix
- **速度会话生命周期**：页面在 0% 位置重新可见时同样重启速度采样；`beforeunload` 除位置外同步发起阅读时长与速度 flush，降低关闭路径依赖 `visibilitychange` 的数据丢失风险。跳读阈值同时适配 locations 量化步长，图片型 EPUB 的正常翻页不再因稀疏索引单步超过 5% 而反复丢弃速度 session。

### refactor
- 删除与动态 locations break 脱节且最终只显示“估算中”的静态字数 ETA 伪计算；ETA 现在只在真实历史或当前会话样本充分时给出分钟数。

### test
- 真实浏览器 E2E 增加速度生命周期：真实翻页并模拟三分钟阅读，触发 visibility flush，关闭重开后校验 `bookMeta.speed`、阅读时长和历史 ETA；新增稀疏 locations 速度回归，单元覆盖增至 271 项。
