...
3. **Manifest 暴露面偏宽** ✅ *v1.2.7 已修复*
- `web_accessible_resources` 对 `<all_urls>` 暴露 `lib/*`，超最小权限。
- 修复：将 `matches` 收敛至 `chrome-extension://*/*`。

4. **`showLoadError` XSS 风险** ✅ *v1.2.6 已修复*
- `err.message` 直接拼入 `innerHTML`，仅过滤 `<`，存在 XSS 注入面。
- 修复：改用纯 DOM API（`textContent` + `createElement`）。

5. **`getLocations`/`removeLocations` 不带 IndexedDB 版本号** ✅ *v1.2.6 已修复*
- 新用户首次访问时创建空 V1 数据库，进度缓存永久读取失败。
- 修复：两处均补全为 `open('EpubReaderDB', 3)` + `onupgradeneeded`。

6. **`window mousedown` 监听器累积** ✅ *v1.2.6 已修复*
- 匿名函数在 `setBookDetails()` 中重复注册，每次切换布局叠加一个。
- 修复：提取为具名函数，移至 `init()` 仅注册一次。
...
6. **innerHTML 使用模式不统一，存在未来 XSS 回归风险** *(部分已修复)*
- `showLoadError` ✅ *v1.2.6 已修复*；其余输入点需持续巡检。
...
8. **阅读时长关闭丢失** ✅ *v1.2.6 已修复*
- 计时器 10 秒写一次，关闭标签页丢失最多 9 秒。
- 修复：`visibilitychange` 立即 flush。

9. **面板 overlay 竞态** ✅ *v1.2.7 已修复*
- TOC/Search/Bookmarks 共享 overlay 但各自独立管理，互相干扰。
- 修复：统一 `closeAllPanels()` 入口 + 各面板关闭时检查其余面板状态。

10. **封面 ObjectURL 内存泄漏** ✅ *v1.2.7 已修复*
- `createObjectURL` 后从未 revoke，刷新书架持续累积。
- 修复：`img.onload/onerror` 后立即 `revokeObjectURL`。

## P2（持续改进）

8. ETA 估算模型静态化，跨书籍偏差大。**☐ 待 Phase D**
9. `URL.createObjectURL` 生命周期管理 ✅ *v1.2.7 已修复（home.js / popup.js）*
10. 进度条高频触发 `rendition.display()` ✅ *v1.2.7 已修复（input/change 分离）*
11. `manifest.json` 权限过宽 ✅ *v1.2.7 已修复*
12. 自动化回归矩阵缺失。**☐ 待 Phase C**
...
> **✅ = v1.2.6/v1.2.7 已落地；☐ = 待后续 Phase C/D 推进**

- [x] 收敛 `web_accessible_resources` 到最小权限 *(P2-E — v1.2.7)*
- [x] 修复 `showLoadError` XSS 风险，改用 DOM API *(P0-B — v1.2.6)*
- [x] 修复 `getLocations`/`removeLocations` 不带版本号问题 *(P0-A — v1.2.6)*
- [x] 修复 `window mousedown` 监听器累积，提取具名函数到 `init()` *(P0-C — v1.2.6)*
- [x] 新增 `visibilitychange` 立即保存阅读时长 *(P1-A — v1.2.6)*
- [x] 统一面板 overlay 管理（TOC/Search/Bookmarks 三路协同）*(P1-B / P1-C — v1.2.7)*
- [x] 统一 `gap` 值（openBook 与 setLayout 均为 48）*(P1-D — v1.2.7)*
- [x] 修复封面 ObjectURL 泄漏，`img.onload/onerror` 后立即 revoke *(P1-E — v1.2.7)*
- [x] popup 删除书籍改为 `removeBook()` 完整级联清理 *(P1-G — v1.2.6/v1.2.7)*
- [x] 进度条 `input`/`change` 事件分离，消除拖动白屏 *(P2-C — v1.2.7)*
- [ ] 统一注解类型常量，修复搜索高亮清理链路 *(Phase C)*
- [ ] BookId 升级为内容指纹（SHA-256）并完成兼容迁移 *(Phase D)*
- [ ] 实施 `navTxnId` + 位置写节流与写队列 *(Phase C)*
- [ ] 建立统一模块生命周期（mount/unmount/dispose）*(Phase C)*
- [ ] 抽象 `db-gateway` + 连接复用 + migration tests *(Phase C)*
- [ ] 错误可观测：ring buffer + 轻提示 + 重试 *(Phase C)*
- [ ] 大列表性能：虚拟化/分页 + 增量更新 *(Phase C)*
- [ ] 资源释放治理：listeners/timers/objectURL/annotations *(Phase C)*
- [ ] 性能埋点与预算：首屏、搜索、导航、写队列深度 *(Phase C)*
