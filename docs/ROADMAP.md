...
> 最后更新：2026-03-12（v1.9.2 封版，2.x 规划详细化；补充 Annotations 算法深度优化专项）
...
### v2.3.0 — Annotations 算法深度对齐（计划 4～6 工作日）

> 本版本基于对 Calibre/KOReader 注释识别算法的逆向分析报告，针对 `annotations.js` 的已知识别短板进行系统性补强。
...
**方案**：在 `isFootnoteLink` Stage 3 末尾，当 href 通过前三阶段未能得出确定结论时，调用 `contents.window.getComputedStyle(link)` 读取 `verticalAlign`，若值为 `super`、`sub`、`top`、`bottom` 则视为强正向信号。需在 `hookRendition` 回调中将 `contents.window` 传入 context，或在 Stage 3 中通过 `link.ownerDocument.defaultView` 获取。
...
Step B（v2.4.0 或 AN-5 中集成）：跨文档场景引入 spine index 比对——若 `sectionHref` 对应的 spine 位置在当前 section 之前，且链接文本是返回类字符，提升 `isBackLink` 判定信心。
...
**方案**：在 `Annotations` 对象上新增 `_contentCache: Map<href, {html, timestamp}>`，容量上限 50 条（LRU 淘汰），TTL = book 生命周期（`setBook` 时清空）。
...
**方案**：提取为 `_hasSup(link)` 私有方法，并在 `_buildDocContext` 阶段或 hookRendition 的 per-link 循环中一次性完成计算，结果存入 WeakMap，避免两个函数重复查询。
...
**方案**：将 `BLOCK` 提升为模块级冻结 `Set`（`const _BLOCK_TAGS = new Set([...])`），`Set.has` 为 O(1)，同时避免每次调用的数组分配。
...
`showFootnote` 的 last-resort 降级路径直接拼接带 inline style 的 HTML 字符串，与项目 v1.9.2 确立的"style.* 全量迁移为 class"约束不一致，也绕过了 `_displayContent` 本身的 inline handler 清洗逻辑（虽然 `var()` 无安全风险，但先例危险）。

**方案**：将该 `<p>` 改为使用 CSS class（如 `annotation-fallback-hint`），样式移入 `reader.css`，字符串改为 `<p class="annotation-fallback-hint">点击下方链接查看注释内容</p>`。
...
`showFootnote` 和 `_loadFromBook` 中均出现 `href.split('#')` 或 `href.includes('#')` 的片段解析，`_compensatePaginationOffset` 中也重复 `href.split('#').pop()`。href 解析逻辑碎片化，修改时容易遗漏。

**方案**：提取 `_parseHref(href)` 纯函数，返回 `{ sectionHref, fragment }`，统一在 `showFootnote` 入口调用一次，结果向下传递。
...
监听挂在全局 `document` 上，且该监听器永不移除（无对应 `removeEventListener`）。当 `hookRendition` 被多次调用重建渲染器时，`init()` 不会重复执行（设计上合理），但若未来 `Annotations` 需要支持多实例或销毁重建，将产生监听器泄漏。此外，当前无法与其他模块（如 TOC、Search 均监听 Escape）协调关闭顺序。

**方案**（分两步）：v2.3.0 将箭头函数改为具名方法 `_onKeyDown`，以便将来可精确移除；v2.0.0 R-2 生命周期接口落地后，在 `unmount()` 中统一 `removeEventListener`。
...
Method 4 对整个 spine 串行 load/unload，在 spine 数百甚至上千条目的书籍（如技术手册、词典类 EPUB）中，取消令牌检查频率足够但串行 await 本身在极端场景下会积累数秒延迟。且 `this.book.load.bind(this.book)` 在每次循环迭代中重复 `.bind()` 创建新函数对象，可提取为循环外变量。

**方案**：① 在循环外 `const bookLoad = this.book.load.bind(this.book)` 避免重复 bind；② 新增早退条件：若当前书籍 spine 长度 > 200 且 targetId 为空，直接跳过 Method 4（无 targetId 的暴力扫描无意义）；③ 与 AN-5 缓存协同，已命中缓存时完全不进入该路径。

#### AN-C8：`_isTocList` 阈值与 `_RE.noteCls` 覆盖范围缺乏注释说明（D-2026-24）

`_isTocList` 使用 `items.length < 3` 和 `>= 0.6` 两个阈值，`_RE.noteCls` / `_RE.noteFragPos` 的正则词汇均为经验积累，但均无参考依据注释，后续维护者难以判断是否可调整。

**方案**：在各处添加行内注释，说明阈值来源（如"< 3 项的列表多为作者标注/图例，非目录"、"0.6 经验阈值来自对 100 本 EPUB 样本的统计"），正则词汇表附上典型来源书籍格式（如 Adobe InDesign 导出、Calibre 转换等）。属于文档治理项，不涉及逻辑变更。
...
- AN-C7 修复：`this.book.load.bind` 提取至循环外；Method 4 在 spine > 200 且无 targetId 时提前退出。
...
| 🔵 P3 | D-2026-11 | CSS `vertical-align` 替代 `<sup>` 的注释链接漏判 | v2.3.0 | 📋 已规划 |
| 🔵 P3 | D-2026-12 | 孤立性链接（父块唯一内容）缺乏专项排他检查 | v2.3.0 | 📋 已规划 |
...
| 🔵 P3 | D-2026-16 | noteTextMarker 支持 4 位数字，年份链接存在误判风险 | v2.4.0 | 📋 已规划 |
| 🔵 P3 | D-2026-17 | `isBackLink`/`isFootnoteLink` 重复 sup DOM 查询，缺 `_hasSup()` 公共方法 | v2.3.0 | 📋 已规划 |
...
| 🔵 P3 | D-2026-21 | `showFootnote`/`_loadFromBook`/`_compensatePaginationOffset` href 解析碎片化 | v2.3.0 | 📋 已规划 |
| 🔵 P3 | D-2026-22 | `init()` Escape 键监听挂全局 document 且永不移除，与 v2.0.0 生命周期接口冲突 | v2.3.0 | 📋 已规划 |
| 🔵 P3 | D-2026-23 | `_loadFromBook` Method 4 循环内重复 `.bind()`，极端 spine 场景无提前退出 | v2.3.0 | 📋 已规划 |
| 🔵 P3 | D-2026-24 | `_isTocList` 阈值与 `_RE` 正则词汇缺乏来源注释，后续维护成本高 | v2.3.0 | 📋 已规划 |
