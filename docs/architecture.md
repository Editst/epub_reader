...
**书架渲染约束**：home 页书籍卡片可流式渲染，但必须按 `recentBooks` 原始索引替换对应骨架，不能按封面/元数据异步完成顺序 append。

**首页异步错误隔离约束**：home 页主题和书架视图切换应先更新 UI；偏好读取/保存、书架刷新、标注刷新、删除和导出失败只能记录告警，不得阻断后续交互绑定或留下未处理 Promise 拒绝。
